...
public:
	...
	std::string to_string() const override
	{
		...
		oss << '<' << (m_precedence == P_UNARY ? "unary" : "binary")
			<< " operator " << m_name << '>';
		return oss.str();
	}

	void add_implementation(const FunctionImplementationPtr & implementation)
...
public:
	...
	std::pair<size_t, OperatorPtr> find(std::string_view symbol, bool is_unary = false) const;
